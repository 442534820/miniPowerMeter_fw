...
const font_t* UI12864_FontSave(const font_t *font)
{
	const font_t *ret = ui_font;
	ui_font = font;

	return ret;
}

void UI12864_FontRestore(const font_t *font)
{
	ui_font = font;
}
...
void UI12864_PutChar(uint8_t Page, uint8_t Column, char Char)
{
	...
	if ((Char < ui_font->begin) || (Char > ui_font->end))
		return;
	h = (ui_font->height - 1) / 8 + 1;
	w = ui_font->width;
	font_ptr = &((uint8_t*)ui_font->data)[(Char - ui_font->begin) * w * h];
	for (j=0; j<h; j++) {
		for (i=0; i<w; i++) {
